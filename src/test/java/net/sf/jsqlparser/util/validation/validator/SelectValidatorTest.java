...
public class SelectValidatorTest extends ValidationTestAsserts {
    ...
    public void testValidationFetchAndOffset() throws JSQLParserException {
        for (String sql : Arrays.asList("SELECT * FROM mytable t WHERE t.col = 9 ORDER BY t.id FETCH FIRST 5 ROWS ONLY", "SELECT * FROM mytable t WHERE t.col = 9 ORDER BY t.id OFFSET 3 ROWS", "SELECT * FROM mytable t WHERE t.col = 9 ORDER BY t.id OFFSET 3 ROWS FETCH NEXT 5 ROWS ONLY")) {
    ...
    public void testValidationSqlIntersect() throws Exception {
        ...
        validateNoErrors(sql, 1, DatabaseType.ORACLE, DatabaseType.SQLSERVER, DatabaseType.MARIADB, DatabaseType.POSTGRESQL, DatabaseType.H2);
    }
    ...
    public void testValidationForUpdateNoWait() throws JSQLParserException {
        ...
        validateNoErrors(sql, 1, DatabaseType.ORACLE, DatabaseType.MARIADB, DatabaseType.POSTGRESQL, DatabaseType.MYSQL);
    }
    ...
    public void testValidationJoin() throws JSQLParserException {
        for (String sql : Arrays.asList("SELECT t1.col, t2.col, t1.id FROM tab1 t1, tab2 t2 WHERE t1.id = t2.id", "SELECT t1.col, t2.col, t1.id FROM tab1 t1 JOIN tab2 t2 ON t1.id = t2.id", "SELECT t1.col, t2.col, t1.id FROM tab1 t1 INNER JOIN tab2 t2 ON t1.id = t2.id")) {
            validateNoErrors(sql, 1, DatabaseType.DATABASES);
        }
    }
    ...
    public void testValidationWithRecursive() throws JSQLParserException {
        ...
        validateNoErrors(statement, 1, DatabaseType.H2, DatabaseType.MARIADB, DatabaseType.MYSQL, DatabaseType.SQLSERVER, DatabaseType.POSTGRESQL);
    ...
    public void testValidatePivotWithAlias() throws JSQLParserException {
        validateNoErrors("SELECT * FROM (SELECT * FROM mytable LEFT JOIN mytable2 ON Factor_ID = Id) f PIVOT (max(f.value) FOR f.factoryCode IN (ZD, COD, SW, PH))", 1, DatabaseType.SQLSERVER);
    }
    ...
    public void testValidateUnPivot() throws JSQLParserException {
        validateNoErrors("select * from pivot_table unpivot (yearly_total for order_mode in (store as 'direct', internet as 'online')) order by year, order_mode", 1, DatabaseType.SQLSERVER);
    }

    @Test
    public void testValidateSubJoin() throws JSQLParserException {
        validateNoErrors("SELECT * FROM ((tabc c INNER JOIN tabn n ON n.ref = c.id) INNER JOIN taba a ON a.REF = c.id)", 1, DatabaseType.SQLSERVER);
    }
    ...
    public void testValidateLateral() throws JSQLParserException {
        validateNoErrors("SELECT O.ORDERID, O.CUSTNAME, OL.LINETOTAL FROM ORDERS AS O, LATERAL(SELECT SUM(NETAMT) AS LINETOTAL FROM ORDERLINES AS LINES WHERE LINES.ORDERID = O.ORDERID) AS OL", 1, DatabaseType.POSTGRESQL, DatabaseType.ORACLE);
    }

    @Test
    public void testValidateIssue1502() throws JSQLParserException {
        validateNoErrors("select b.id, name ,(select name from Blog where name = 'sadf') as name2 " + ", category, owner, b.update_time " + "from Blog as b " + "left join Content " + "ON b.id = Content.blog_id " + "where name = 'sadf' order by Content.title desc", 1, DatabaseType.POSTGRESQL);
    }
}
